...
int main()
{
    ...
    rc::check("Adding a number to itself results in an even number.", [](const int a)
        {
            RC_ASSERT((a + a) % 2 == 0);
        });
    ...
    rc::check("Adding a number to itself results in a number that is greater or equal.", [](const int a)
        {
            RC_ASSERT(a <= a + a);
        });
    ...
    rc::check("Adding a positive number to itself results in a bigger number.", [](const int a)
        {
            RC_PRE(a >= 0);
            RC_ASSERT(a <= a + a);
        });
